...
def dict_to_tf_example(image_path,
  ...
  with tf.io.gfile.GFile(label_path, 'rb') as fid:
...
def create_tf_record(output_filename,
  ...
  for file in os.listdir(image_dir):
    idx += 1
    if idx % 500 == 0:
      print('On image %d of %d', idx, 2)
    image_path = os.path.join(image_dir, file[:-4] + '.jpg')
    label_path = os.path.join(label_dir, file[:-4] + '.png')

    if not os.path.exists(image_path):
      print('Could not find %s, ignoring example.', image_path)
      continue
    elif not os.path.exists(label_path):
      print('Could not find %s, ignoring example.', label_path)
      continue
    ...
    try:
      ...
      print('Invalid example: %s, ignoring.', example)
...
def main(unused_argv):
  ...
  print("Reading from VOC dataset")
